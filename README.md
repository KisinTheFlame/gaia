...
- `packages/cli` -> `@kisinwen/gaia-cli`（开发期命令行工具）
...
```ts
import { deleteConfig, getConfig, initializeGaiaClient, setConfig } from "@kisinwen/gaia-client";
...
## CLI 使用（@kisinwen/gaia-cli）

安装到业务项目：

```bash
npm install -D @kisinwen/gaia-cli
```

CLI 默认读取当前工作目录下的 `gaia.config.yml`，也支持通过 `--config` 指定配置文件：

```bash
npx gaia search payment
npx gaia get payment.timeout
npx gaia set payment.timeout 3000
npx gaia set feature.flags '{"checkoutV2":true,"gray":20}' --json
npx gaia delete payment.timeout
npx gaia get payment.timeout --config ./config/gaia.config.yml
```

CLI 规则：

- 命令：`set`、`get`、`delete`、`search`
- `search` 仅输出匹配到的配置 key；无结果时输出 `未找到匹配配置`
- `set --json` 会先校验 JSON，再按 2 空格缩进格式化后写入
- 默认输出为人类可读文本，失败信息输出到 stderr
...
- 触发：推送 `v*` tag
- 发布包：`@kisinwen/gaia-shared`、`@kisinwen/gaia-client`、`@kisinwen/gaia-cli`
