...
if [ "$1" = "migrate" ] && [ "${2:-}" = "dev" ]; then
  ...
  PG_CONTAINER_NAME="gaia-prisma-migrate-$(date +%s)-$$"
  PG_USER="postgres"
  PG_PASSWORD="postgres"
  PG_DB="gaia_migrate_tmp"
  ...
  docker compose run --rm --no-deps \
    ...
    gaia-config-center node_modules/prisma/build/index.js "${prisma_args[@]}"
  exit 0
fi

docker compose run --rm --no-deps gaia-config-center node_modules/prisma/build/index.js "$@"
